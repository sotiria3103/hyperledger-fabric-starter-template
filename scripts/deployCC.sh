...
if [ "$CC_SRC_PATH" = "NA" ]; then
  ...
  if [ "$CC_NAME" = "trapezePolicy" ]; then
    println $'\e[0;32m'policy-chaincode$'\e[0m' chaincode
    CC_SRC_PATH="./policy-chaincode"
    ...
    fatalln "The chaincode name ${CC_NAME} is not supported by this script. Supported chaincode names are: trapezePolicy, events, ledger, private, sbe, secured"
  fi
